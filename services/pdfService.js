...
/**
 * יוצר תבנית HTML עבור דו"ח הביקורת
 * @param {object} reportData - אובייקט המכיל את כל נתוני הדו"ח (report, business, inspector)
 * @returns {string} - מחרוזת HTML
 */
const createReportHTML = (reportData) => {
  ...
  return `
    ...
    <head>
      ...
      <style>
        body {
          /* גופן Arial הוא גופן בטוח שתומך בעברית ברוב המערכות */
          /* ב-Dockerfile נוודא התקנה של גופנים מתאימים עבור Cloud Run */
        ...
        .findings {
          white-space: pre-wrap; /* שומר על מעברי שורה מה-textarea */
...
/**
 * יוצר PDF מנתוני דו"ח
 * @param {object} reportData - אובייקט המכיל את הדו"ח, העסק והמפקח
 * @returns {Promise<Buffer>} - באפר של קובץ ה-PDF
 */
const generateReportPDF = async (reportData) => {
  ...
  try {
    ...
    const pdfBuffer = await page.pdf({
      ...
      `,
      headerTemplate: '<div></div>' // פוטר בלבד
    });
